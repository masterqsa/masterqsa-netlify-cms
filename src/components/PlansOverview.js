        ...
        return (
          <div className="offer" key={plan.name}>
            <div className={`offer__mainInfoContainer`}>
              {variant === 'DEFAULT' && (
                <div className="offer__mainInfoBackground" style={{ backgroundColor: plan.color }} />
              )}
              <p className={`offer__mainInfo has-text-centered is-size-4 has-text-weight-semibold ${variant === 'HOME' ? 'has-text-black' : 'has-text-white'}`}>
                {plan.name}
                <span className="is-block is-size-1">
                  ${(plan.amount / 100).toLocaleString('en-US')}
                </span>
                <span className={`is-block is-size-6 ${variant === 'HOME' ? 'has-text-grey' : 'has-text-grey-lighter'}`}>
                  {plan.terms}
                </span>
              </p>
            </div>
